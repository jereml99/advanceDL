...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        if schedule == 'linear':
            return torch.linspace(start=self.beta_start, end=self.beta_end, steps= self.T) # HINT: use torch.linspace to create a linear schedule from beta_start to beta_end
    ...
    def p_mean_std(self, model, x_t, t):
        ...
        mean = torch.sqrt(1/alpha)*(x_t - (beta/torch.sqrt(1-alpha_bar)*predicted_noise)) # HINT: calculate the mean of the distribution p(x_{t-1} | x_t). See Eq. 11 in the ddpm paper at page 4
        std = torch.sqrt(beta)
    ...
    def p_sample(self, model, x_t, t):
        ...
        # Ensure `mask` is broadcastable to `x_t`. Assume `t` has shape `(batch_size,)`.
        mask = (t > 1).float().view(-1, 1, 1, 1)  # Reshape mask to be compatible with `x_t` dimensions.
        noise = torch.randn_like(x_t)
        noise = noise * mask  # Element-wise multiplication that should now work without size mismatch.
